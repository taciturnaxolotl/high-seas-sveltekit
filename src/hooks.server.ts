...
const personMiddleware: Handle = async ({ event, resolve }) => {
    ...
    const person = await (async () => {
        ...
        const people = await airtable('people').select({
    ...
    event.locals.person = {
        ...
        shipsAwaitingVoteRequirement: person.fields.ships_awaiting_vote_requirement as number,
...
const loadShipsMiddleware: Handle = async ({ event, resolve }) => {
    const isShipyardPage = event.url.pathname.startsWith('/shipyard') || event.url.pathname.startsWith('/debug');
